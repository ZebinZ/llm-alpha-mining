...
from alpha_research.core.frequency import FrequencySpec
from alpha_research.factors.spec import (
    AggregationSpec,
    ComplexityBudget,
    FactorProvenance,
    FactorSpec,
...
def factor_spec_from_v5_candidate(
    ...
    *,
    ...
    """Map a frozen V5 proposal into the governed Phase 2 definition.

    Data, universe and operator bindings are intentionally supplied by the
    caller: the legacy candidate wire model never contained enough evidence to
    infer them safely.
    ...
    if not economic_rationale.strip() or not falsification_criterion.strip():
        raise ValueError("V5 compatibility mapping requires research rationale")
    ...
    if candidate_frequency is not expected:
        raise ValueError("V5 candidate/frequency binding differs")
    if len(parent_factor_hashes) != len(candidate.parent_ids):
        raise ValueError("V5 parent ids require explicit registered factor hashes")
...
__all__ = ["factor_spec_from_v5_candidate"]
