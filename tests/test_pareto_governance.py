...
def test_objective_contract_is_typed_frozen_complete_and_fail_closed() -> None:
    ...
    with pytest.raises(ParetoGovernanceError, match="must use direction"):
        ParetoObjectiveSpec(ParetoObjective.TURNOVER, ObjectiveDirection.MAXIMIZE)
...
def test_epsilon_dominance_handles_maximize_and_minimize_without_scalarization() -> (
    None
):
    ...
    inside_epsilon = _observation("inside", neutral_ic_strength=0.025, turnover=0.405)
    meaningful = _observation("meaningful", neutral_ic_strength=0.031, turnover=0.405)
...
def test_round_and_campaign_frontiers_preserve_real_tradeoffs() -> None:
    ...
    assert tuple(
        item.candidate_id for item in pareto_frontier(first_round.observations)
    ) == (
        "cheap",
        "strong",
    )
    ...
    empty_latest = build_campaign_frontier((first_round, ParetoRoundObservation(2, ())))
    assert empty_latest.through_round == 2


def test_diverse_shortlist_uses_objective_queues_and_fails_closed_on_correlation() -> (
    None
):
    observations = (
        ...
        _observation("novel", neutral_ic_strength=0.02, turnover=0.30, novelty=0.99),
    )
...
def test_checkpoint_is_hash_bound_verifiable_and_restorable() -> None:
    ...
    assert (
        restored_controller.checkpoint().checkpoint_hash == checkpoint.checkpoint_hash
    )
