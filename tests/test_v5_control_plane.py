...
from factor_production.v5.artifacts import ArtifactManifest, ArtifactStore
from factor_production.v5.cli import initialize_workspace, verify_workspace, workspace_status
from factor_production.v5.domain.enums import CandidateState, RunState, ScoreVisibility
from factor_production.v5.domain.models import CandidateSpecError, CandidateSpecV5
from factor_production.v5.orchestration.budget import BudgetController, BudgetExceeded, BudgetLimits
...
from factor_production.v5.protocol import ProtocolError, V5Protocol, default_protocol
from factor_production.v5.providers import MockProvider, ProposalContext, ReplayProvider
from factor_production.v5.scorers import FileDropScorer, HoldoutIsolationError, MockScorer
from factor_production.v5.scorers.file_drop import DROP_RESULT_SCHEMA, FileDropError
...
def test_candidate_spec_is_single_strict_immutable_and_content_hashed() -> None:
    ...
    second = CandidateSpecV5.from_dict(
    ...
    with pytest.raises(CandidateSpecError, match="unknown CandidateSpecV5 fields"):
        CandidateSpecV5.from_dict({**first.to_dict(), "local_score": 1.0})
    with pytest.raises(CandidateSpecError, match="external holdout field is forbidden"):
        CandidateSpecV5.from_dict(
...
def test_sqlite_repository_enforces_protocol_and_state_machine(tmp_path: Path) -> None:
    ...
    with SQLiteRepository(tmp_path / "control.sqlite3") as repository:
        ...
        record = repository.transition_candidate(
            "run", candidate.candidate_id, CandidateState.PROPOSED, reason="provider returned"
        )
        ...
        with pytest.raises(InvalidStateTransition, match="illegal state transition"):
            repository.transition_candidate(
                "run", candidate.candidate_id, CandidateState.SELECTED, reason="skip all gates"
            )


def test_artifact_manifest_detects_post_freeze_tampering(tmp_path: Path) -> None:
    store = ArtifactStore(tmp_path)
    record = store.put_bytes("protocol", b'{"frozen":true}\n', media_type="application/json")
...
def test_file_drop_results_are_hash_bound_and_cannot_become_feedback(tmp_path: Path) -> None:
    protocol = default_protocol("file-drop")
    candidate = _candidate(protocol.content_hash)
    scorer = FileDropScorer()
    request_path = tmp_path / "request.json"
    request = scorer.export_request((candidate,), request_path, protocol_hash=protocol.content_hash)
...
def test_local_mock_scores_can_drive_feedback_but_external_scorer_cannot_inline() -> None:
    protocol = default_protocol("scores")
    candidate = _candidate(protocol.content_hash)
    local = MockScorer({candidate.candidate_id: {"robust_score": 0.12}}).score((candidate,))
...
def test_budget_is_reserved_fail_closed_and_stopping_uses_local_metric() -> None:
    ...
    controller = StopController(StoppingConfig(patience_generations=2, min_improvement=0.01))
    first = controller.observe(ObjectiveObservation(0, 0.10, "local_score"), stopping_budget)
    second = controller.observe(ObjectiveObservation(1, 0.105, "local_score"), stopping_budget)
    third = controller.observe(ObjectiveObservation(2, 0.106, "local_score"), stopping_budget)
