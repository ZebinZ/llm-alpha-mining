...
from alpha_research.costs import CostCalibration, CostModel
...
def _fixture(
    *,
    ...
    execution_reason = pd.DataFrame(
        "tradable", index=timestamps, columns=securities
    )
    if not tradable:
...
def test_nontradable_reason_is_preserved_in_order_audit(reason: str) -> None:
    timestamps, portfolio, data = _fixture(
        tradable=False, nontradable_reason=reason
    )
    cost = CostModel.zero()
    result = BacktestEngine().run(_spec(portfolio, cost, data), portfolio, cost, data)
...
def test_backtest_bindings_fail_closed() -> None:
    ...
    with pytest.raises(ValueError, match="weights binding"):
        BacktestEngine().run(
            replace(
                _spec(portfolio, cost, data), portfolio_weights_hash="f" * 64
            ),
            portfolio,
            cost,
            data,
...
def test_production_execution_uses_real_open_clock_and_rejects_future_tradability() -> None:
    timestamps, portfolio, data = _fixture(production_ready=True)
    cost = CostModel.zero()
    result = BacktestEngine().run(
        _spec(portfolio, cost, data), portfolio, cost, data
    )
    first = result.orders.loc[
        result.orders["execution_session_timestamp"] == timestamps[1]
    ].iloc[0]
    assert first["execution_timestamp"] == _event_timestamp(
        timestamps[1], "09:30:00"
    )

    availability = {
        name: panel.copy() for name, panel in data.known_at_panels.items()
    }
    ...
    with pytest.raises(ValueError, match="future backtest availability:execution_tradability"):
        BacktestEngine().run(
            _spec(portfolio, cost, future), portfolio, cost, future
        )
