...
from factor_production.v5.llm.agents import StructuredAgent, StructuredCallExecutor
...
from factor_production.v5.llm.transport.base import TransportError
...
def _config() -> LiveProviderConfig:
    return LiveProviderConfig(
        ...
        api_key_env="PHASE4_LIVE_API_KEY",
...
def _build_request(policy: LLMPolicy):
    ...
    executor_stub = cast(
        StructuredCallExecutor, SimpleNamespace(policy=policy)
    )
...
def test_live_transport_runs_through_v5_ledger_and_strict_schema(
    ...
    monkeypatch.setenv("PHASE4_LIVE_API_KEY", "test-runtime-credential")
...
def test_missing_credential_fails_before_any_network_dispatch(monkeypatch) -> None:
    monkeypatch.delenv("PHASE4_LIVE_API_KEY", raising=False)
    policy = _policy()
