...
from factor_eval_minute.expression_engine import ExpressionEngine
...
def test_safe_interpreter_matches_legacy_engine_for_allowed_formula() -> None:
    expression = "CsRank(TsMean(Returns, 2))"
    fields = _fields()
    expected = ExpressionEngine().evaluate(expression, fields)
