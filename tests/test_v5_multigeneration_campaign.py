...
from factor_production.v5.providers import MockProvider, SanitizedFeedback
...
def test_legacy_candidate_wire_and_hash_are_unchanged() -> None:
    ...
    assert legacy.content_hash == "3968ec4fa8dcf944419e674fad1699f89ac1d04d10a41e6ffea40015120933a9"
    ...
    assert CandidateSpecV5.from_dict(legacy.to_dict()).content_hash == legacy.content_hash
...
def test_explicit_lineage_separates_round_from_depth_and_roundtrips() -> None:
    ...
    assert CandidateSpecV5.from_dict(payload) == late_root
    with pytest.raises(CandidateSpecError, match="supplied together"):
        CandidateSpecV5.from_dict({**payload, "lineage_depth": None})
...
def test_campaign_engine_accepts_late_roots_mutations_and_persists_ledger(
    ...
    with SQLiteRepository(tmp_path / "control.sqlite3") as repository:
        ...
        assert repository.get_candidate("run", "mutation").state is CandidateState.PROPOSED
        assert repository.verify_integrity("run") == ()
        assert engine.next_campaign_round == 2
        assert len(engine.content_hash) == 64


def test_campaign_engine_rejects_same_signal_resubmitted_with_other_sign(tmp_path: Path) -> None:
    root = _candidate("root", "CsRank(TsMean(Returns, 5))")
    duplicate = _candidate(
        "duplicate",
        "CsRank( TsMean(Returns, 5) )",
