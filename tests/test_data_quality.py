...
from llm_alpha_mining.research.core.frequency import (
    AvailabilitySpec,
    FrequencySpec,
    SessionSpec,
    TradingCalendar,
)
from llm_alpha_mining.research.core.hashing import hash_file
from llm_alpha_mining.research.data.quality import (
    DataQualityEngine,
    QualityDecision,
    QuarantineStore,
)
...
def test_duplicate_missing_infinity_and_wrong_session_are_quarantined(
    ...
    bad = bad.sort_values(["timestamp", "security"], kind="stable").reset_index(
        drop=True
    )
    batch = _batch(schema, dataset, snapshot, request, bad)
    report = DataQualityEngine().evaluate(batch, schema=schema, calendar=calendar)
