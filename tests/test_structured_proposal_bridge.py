...
from llm_alpha_mining.research.experiments import (
    DataPartition,
    ExperimentProfile,
    ExperimentSpec,
    ResourceBudget,
...
from llm_alpha_mining.mining.llm.domain import LLMRole, Usage
from llm_alpha_mining.mining.llm.ledger import CallLedger
from llm_alpha_mining.mining.llm.policy import LLMPolicy
from llm_alpha_mining.mining.llm.schemas import schema_hash
...
def _setup(tmp_path):
    ...
    spec = ExperimentSpec(
        experiment_id="research-llm-bridge",
        version="1",
...
def test_v5_llm_bridge_binds_task_context_request_response_usage_and_ledger(
    tmp_path,
) -> None:
    spec, context, task, transport, executor, transport_hash = _setup(tmp_path)
    bridge = StructuredProposalBridge(executor, transport_authority_hash=transport_hash)
    first = bridge.propose(spec=spec, task=task, context=context, requested_count=1)
    second = bridge.propose(spec=spec, task=task, context=context, requested_count=1)
    ...
    assert first.evidence.output_semantic_hashes == (first.proposals[0].semantic_hash,)
