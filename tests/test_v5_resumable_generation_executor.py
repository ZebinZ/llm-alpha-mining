...
def test_full_run_checkpoints_in_order_and_success_is_idempotent(tmp_path: Path) -> None:
    calls: list[tuple[str, str, tuple[str, ...]]] = []
    ...
    with _store(tmp_path) as store:
        ...
        assert tuple(item.stage for item in store.list_checkpoints(1)) == CHECKPOINT_STAGES
...
def test_running_attempt_with_all_checkpoints_finishes_without_stage_reexecution(
    ...
    with _store(tmp_path) as store:
        ...
        assert result.result_hash == store.get_checkpoint(
            attempt.attempt_id, "metrics"
        ).sha256
