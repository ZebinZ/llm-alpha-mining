...
from factor_production.v5.llm.agents import (
    CriticAgent,
    ProposerAgent,
    StructuredCallExecutor,
    StructuredCallFailed,
)
from factor_production.v5.llm.budget import LLMBudget, LLMBudgetExceeded, LLMBudgetLimits
...
from factor_production.v5.llm.ledger import CallLedger, IndeterminateCallError, LedgerIntegrityError
from factor_production.v5.llm.panel import AlphaResearchPanel
from factor_production.v5.llm.policy import DEFAULT_PROMPTS, LLMPolicy
...
from factor_production.v5.llm.transport.replay import (
...
from factor_production.v5.orchestration.hypotheses import semantic_signal_hash
from factor_production.v5.providers.base import LocalFeedback, ProposalContext
from factor_production.v5.providers.llm_panel import LLMPanelProvider


def _candidate(candidate_id: str, expression: str, fields: list[str]) -> dict[str, object]:
    return {
        ...
        LLMRole.PROPOSER: [
            {
                ...
                "proposals": [
                    _candidate("liquidity_reversal", "Neg(TsMean(Returns, 5))", ["Returns"]),
                    _candidate("impact_blocked", "Div(Returns, Amount)", ["Returns", "Amount"]),
                ],
            }
        ],
        LLMRole.CRITIC: [
...
def _executor(tmp_path: Path, transport, *, policy: LLMPolicy | None = None, name: str = "ledger"):
...
def test_role_parser_is_a_second_typed_pass_and_rejects_cot() -> None:
    ...
    narrated["hypothesis"] = "I considered several formulas and decided this one should work."
...
def test_golden_vertical_slice_risk_veto_and_provider_adapter(tmp_path: Path) -> None:
    ...
    panel = AlphaResearchPanel(executor, max_expression_depth=protocol.max_expression_depth)
...
def test_exact_replay_reproduces_all_four_roles_without_network(tmp_path: Path) -> None:
    ...
    assert [item.to_dict() for item in first.selected] == [item.to_dict() for item in second.selected]
...
def test_replay_cassette_is_content_bound_and_tamper_evident(tmp_path: Path) -> None:
    ...
    path.write_text(path.read_text(encoding="utf-8").replace("liquidity_reversal", "liquidity_tampered"), encoding="utf-8")
...
def test_replay_entry_cannot_persist_unsafe_output_or_metadata(tmp_path: Path) -> None:
    ...
    ).build_request(_context(default_protocol("unsafe-replay-entry").content_hash).to_dict())
    with pytest.raises(UnsafeLLMContext):
        ReplayEntry(
            request=request,
...
def test_finite_retry_uses_same_idempotency_key_and_counts_calls(tmp_path: Path) -> None:
...
def test_timeout_is_indeterminate_and_restart_never_duplicates_call(tmp_path: Path) -> None:
...
def test_crash_after_atomic_reserve_retains_pending_maximum_and_denies_redispatch(tmp_path: Path) -> None:
...
def test_ledger_hash_chain_detects_tampering(tmp_path: Path) -> None:
    ...
    path.write_text(text.replace("call_succeeded", "call_tampered", 1), encoding="utf-8")
...
def test_invalid_or_unsafe_response_is_charged_but_never_persisted(tmp_path: Path) -> None:
    ...
    assert [row["event"] for row in rows] == ["attempt_started", "call_terminal_failure"]
...
def test_role_semantic_failure_is_not_cached_and_is_charged(tmp_path: Path) -> None:
    ...
    from factor_production.v5.llm.agents import RiskAgent

    with pytest.raises(StructuredCallFailed, match="structured_output_invalid"):
        RiskAgent(executor).call(_context(default_protocol("role-invalid").content_hash).to_dict())
...
def test_provider_metadata_and_transport_error_secrets_never_persist(tmp_path: Path) -> None:
    ...
    assert failed_executor.budget.snapshot.cost_microusd == LLMPolicy().max_cost_microusd_per_call
    assert failed_executor.budget.snapshot.pending_reservations == 0


def test_over_reservation_usage_is_terminal_charged_and_cannot_be_cached(tmp_path: Path) -> None:
    ...
    context = _context(protocol.content_hash).to_dict()
    with pytest.raises(StructuredCallFailed, match="provider_usage_exceeded_reservation"):
...
def test_parent_catalog_binds_lineage_without_metrics(tmp_path: Path) -> None:
    ...
    scripts = {
        ...
        LLMRole.CRITIC: [
            {
                "schema_version": "llm-critic-output/v1",
                "reviews": [{"candidate_id": "child_factor", "verdict": "approve", "reason_codes": ["mechanism_supported"]}],
            }
        ],
        LLMRole.RISK: [
            {
                "schema_version": "llm-risk-output/v1",
                "assessments": [{"candidate_id": "child_factor", "decision": "allow", "reason_codes": ["no_blocker"]}],
            }
        ],
        LLMRole.ARBITER: [
            {
                "schema_version": "llm-arbiter-output/v1",
                "decisions": [{"candidate_id": "child_factor", "decision": "select", "reason_codes": ["balanced_mechanism"], "priority": 50}],
            }
        ],
    }
    ...
    empty_provider = LLMPanelProvider(
        protocol=protocol,
        panel=AlphaResearchPanel(_executor(tmp_path, FakeTransport({}), name="missing-parent")),
    )
...
def test_provider_request_identifier_is_hashed_before_any_persistence(tmp_path: Path) -> None:
...
def test_model_allowlist_is_closed_and_hash_bound(tmp_path: Path) -> None:
    ...
    assert executor.ledger.records[-1]["details"]["error_code"] == "response_dlp_rejected"
    assert all(row["event"] != "call_succeeded" for row in executor.ledger.records)
...
def test_sensitive_provider_request_metadata_fails_closed_without_persistence(
    ...
    assert executor.ledger.records[-1]["details"]["error_code"] == "response_dlp_rejected"
    assert (
        executor.budget.snapshot.cost_microusd
        == LLMPolicy().max_cost_microusd_per_call
    )


def test_provider_errors_have_no_raw_exception_chain_or_traceback(tmp_path: Path) -> None:
    ...
    assert transport_error.diagnostic_hash == hashlib.sha256(raw_secret.encode("utf-8")).hexdigest()
    executor = _executor(
        tmp_path,
...
def test_panel_and_campaign_registry_share_one_semantic_identity() -> None:
    ...
    def to_spec(draft: CandidateDraft) -> CandidateSpecV5:
        return CandidateSpecV5(
...
def test_terminal_failure_without_usage_keeps_conservative_reservation(tmp_path: Path) -> None:
    ...
    request = ProposerAgent(executor).build_request(_context(protocol.content_hash).to_dict())
...
def test_live_hex_provider_identifier_is_domain_hashed_not_trusted() -> None:
    ...
    assert StructuredCallResponse.from_dict(response.to_dict()).provider_request_id == expected
...
def test_malformed_transport_return_is_sanitized_terminal_and_conservative(
    ...
    with pytest.raises(StructuredCallFailed, match="malformed_transport_response") as caught:
        ProposerAgent(executor).call(
...
def test_review_coverage_failure_is_terminal_before_success_or_replay_cache(
    ...
    assert executor.ledger.records[-1]["details"]["error_code"] == "structured_output_invalid"
    assert all(row["event"] != "call_succeeded" for row in executor.ledger.records)
...
def test_ema30_contract_binds_schema_descriptor_parent_and_provider(tmp_path: Path) -> None:
    ...
    provider = LLMPanelProvider(
        protocol=protocol,
        panel=AlphaResearchPanel(_executor(tmp_path, FakeTransport(scripts), name="ema30")),
    )
    batch = provider.propose(
        ProposalContext(
            run_id="ema30-run",
...
def test_ledger_pin_head_count_and_bytes_are_one_concurrent_snapshot(
    ...
    import factor_production.v5.llm.ledger as ledger_module
