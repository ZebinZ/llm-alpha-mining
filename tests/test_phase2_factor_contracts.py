...
from alpha_research.factors import (
    ...
    PreprocessKind,
    PreprocessStep,
    factor_spec_from_v5_candidate,
)
from alpha_research.labels import LabelDataView, PriceObservationEvent
...
from factor_production.v5.domain import CandidateSpecV5
...
def _fixture(tmp_path: Path):
    ...
    source.write_bytes(b"phase2-factor-source")
    schema = standard_market_schema(
        schema_id="phase2-factor-data",
        version="1",
    ...
    dataset = DatasetSpec(
        dataset_id="phase2-daily",
        version="1",
        owner="research",
        description="Phase 2 factor contract fixture",
    ...
    timestamps = pd.DatetimeIndex(
        [f"{date} 15:00:00" for date in DATES]
...
def _spec(dataset, snapshot, schema, view, *, expression: str, fields: tuple[str, ...]):
    ...
    return FactorSpec(
        ...
        provenance=FactorProvenance(origin="human", actor_id="phase2-test"),
    )
...
def test_factor_engine_rejects_snapshot_and_field_contract_drift(tmp_path: Path) -> None:
    dataset, snapshot, schema, _, _, view = _fixture(tmp_path)
    spec = _spec(
        dataset,
        snapshot,
...
def test_degraded_security_contract_requires_explicit_research_override(
    ...
    with pytest.raises(PermissionError, match="not_production_ready"):
        FactorDataView.from_market_view(
            market, degraded, required_fields=("Returns",)
        )
    view = FactorDataView.from_market_view(
        market,
        degraded,
...
def test_minute_signal_calendar_daily_aggregation_is_executed_not_just_declared() -> None:
    timestamps = pd.DatetimeIndex(
        [
...
def test_v5_candidate_requires_explicit_data_and_research_bindings(tmp_path: Path) -> None:
    dataset, snapshot, schema, _, _, view = _fixture(tmp_path)
    registry = OperatorRegistry.dataframe_pit_v3(
        view.cross_section_mask, view.history_mask
    )
    candidate = CandidateSpecV5(
        candidate_id="v5_bridge_test",
    ...
    spec = factor_spec_from_v5_candidate(
        candidate,
...
def test_factor_registry_is_append_only_semantically_deduplicated_and_lineaged(
    ...
    with FactorRegistry(tmp_path / "factor-registry.sqlite3") as registry:
        ...
        assert registry.register(
            different_universe, registered_at=CREATED
        ) == different_universe.content_hash

        child = replace(
            parent,
            factor_id="phase2_child",
            version="1",
            expression="CsRank(TsMean(Returns, 2))",
            provenance=FactorProvenance(
                origin="mutation",
                actor_id="phase2-test",
...
def test_factor_registry_batch_is_atomic_idempotent_and_supports_ordered_lineage(
    ...
    with FactorRegistry(path) as registry:
        ...
        assert registry.register_many(
            (parent, child), registered_at=CREATED
        ) == expected
        assert registry.register_many(
            (parent, child), registered_at=CREATED
        ) == expected
...
def test_factor_registry_batch_rolls_back_on_conflict_and_rejects_bad_shape(
    ...
    same_definition = replace(
        first,
        factor_id="phase2_batch_definition_conflict",
        version="2",
    )
    with FactorRegistry(tmp_path / "factor-registry-rollback.sqlite3") as registry:
        with pytest.raises(FactorRegistryConflict, match="executable definition"):
            registry.register_many(
                (first, same_definition), registered_at=CREATED
            )
        assert registry.list_hashes() == ()
