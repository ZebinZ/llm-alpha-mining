...
from factor_production.v5.domain.enums import StopReason
...
class BudgetUsage:
    ...
    def __post_init__(self) -> None:
        for name in ("candidates", "generations_started", "provider_calls", "evaluations", "wall_seconds"):
...
class BudgetController:
    ...
    def _assert_within_limits(self, usage: BudgetUsage) -> None:
        checks = (
            ...
            (usage.generations_started, self.limits.max_generations, StopReason.GENERATION_BUDGET),
            (usage.provider_calls, self.limits.max_provider_calls, StopReason.PROVIDER_CALL_BUDGET),
            (usage.evaluations, self.limits.max_evaluations, StopReason.EVALUATION_BUDGET),
            (usage.wall_seconds, self.limits.max_wall_seconds, StopReason.WALL_TIME_BUDGET),
        )
        for actual, limit, reason in checks:
            if actual > limit:
                raise BudgetExceeded(reason, f"{reason.value} exceeded: {actual}>{limit}")
    ...
    def reserve(
        ...
        *,
        ...
        increments = (candidates, generations, provider_calls, evaluations, wall_seconds)
