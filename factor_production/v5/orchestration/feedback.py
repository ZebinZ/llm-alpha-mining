...
from factor_production.v5.artifacts.hashing import canonical_json_bytes, hash_file
from factor_production.v5.domain.enums import FeedbackDisposition, FeedbackReason
...
def build_feedback_batch(
    *,
    ...
    source_campaign_round: int,
    source_snapshot_sha256: str,
    specs: Iterable[CandidateSpecV5],
    ...
    for raw in gate_decisions:
        ...
        if not isinstance(candidate_id, str) or candidate_id not in by_id:
            raise FeedbackBuildError(f"gate decision references unknown candidate: {candidate_id!r}")
...
def load_gate_decisions_jsonl(path: str | Path) -> tuple[dict[str, object], ...]:
    ...
    for line_number, raw in enumerate(
        ...
        try:
            ...
            raise FeedbackBuildError(f"invalid gate JSON at line {line_number}") from exc
