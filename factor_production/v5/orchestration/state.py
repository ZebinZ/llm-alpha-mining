...
def assert_transition(
    ...
    if target not in transitions[current]:
        raise InvalidStateTransition(f"illegal state transition: {current.value} -> {target.value}")


def assert_candidate_transition(current: CandidateState, target: CandidateState) -> None:
