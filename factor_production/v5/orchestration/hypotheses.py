...
def _expression_structure(expression: str) -> str:
    ...
    try:
        ...
        raise HypothesisRegistryError("invalid_expression_syntax", "invalid expression") from None


def semantic_signal_hash(spec: CandidateSpecV5) -> str:
    ...
    wire = spec.to_dict()
    from factor_production.v5.llm.schemas import canonical_aggregation
...
class HypothesisRegistry:
    ...
    def preflight(self, spec: CandidateSpecV5) -> HypothesisEntry:
        ...
        if spec.proposal_kind is ProposalKind.ROOT:
            ...
            missing = [parent_id for parent_id in spec.parent_ids if parent_id not in self._entries]
            if missing:
                raise HypothesisRegistryError(
                    "missing_parent",
                    ",".join(sorted(missing)),
            ...
            if any(parent.campaign_round >= spec.campaign_round for parent in parent_entries):
                raise HypothesisRegistryError(
    ...
    def register(self, spec: CandidateSpecV5) -> HypothesisEntry:
    ...
    def register_many(self, candidates: Iterable[CandidateSpecV5]) -> tuple[HypothesisEntry, ...]:
        registered: list[HypothesisEntry] = []
        for candidate in sorted(
            candidates,
            key=lambda item: (item.campaign_round, item.lineage_depth, item.candidate_id),
        ):
