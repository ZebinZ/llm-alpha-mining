...
def _load_workspace_descriptor(root: Path) -> tuple[dict[str, Any], str]:
    ...
    try:
        ...
        raise ChildSnapshotIntegrityError(f"cannot load workspace descriptor: {exc}") from exc
    expected = {
        "schema_version",
        "run_id",
        "protocol_hash",
...
def _read_attempt_logical_payload(
    ...
    try:
        ...
        raise ChildSnapshotIntegrityError(f"cannot read child attempt store: {exc}") from exc
    finally:
        connection.close()
    if integrity != "ok":
        raise ChildSnapshotIntegrityError(f"child attempt SQLite integrity failed: {integrity}")
...
def _validate_child_attempt_rows(payload: Mapping[str, Any]) -> None:
    ...
    if not isinstance(metadata_rows, list) or not isinstance(attempts, list) or not isinstance(checkpoints, list):
        raise ChildSnapshotIntegrityError("attempt logical payload lists are invalid")
    metadata = {row.get("key"): row.get("value") for row in metadata_rows if isinstance(row, dict)}
    if len(metadata) != len(metadata_rows):
        raise ChildSnapshotIntegrityError("attempt metadata rows are malformed or duplicated")
    ...
    for row in attempts:
        ...
        if status == "succeeded" and result_hash is None:
            raise ChildSnapshotIntegrityError("successful child attempt lacks result hash")
    for row in checkpoints:
        if not isinstance(row, dict):
            raise ChildSnapshotIntegrityError("child checkpoint row is not an object")
        attempt_id = row.get("attempt_id")
        if attempt_id not in attempt_ids:
            raise ChildSnapshotIntegrityError("checkpoint references unknown child attempt")
        stage = row.get("stage")
        if stage not in _CHECKPOINT_STAGES:
            raise ChildSnapshotIntegrityError(f"unknown child checkpoint stage: {stage!r}")
    ...
    for attempt_id, status in attempt_status.items():
        if status == "succeeded" and len(checkpoint_by_attempt.get(attempt_id, ())) != 4:
            raise ChildSnapshotIntegrityError(
...
class ChildAttemptSnapshot:
    ...
    def _core_dict(self) -> dict[str, Any]:
        return {
            ...
            "attempt_store": {
                ...
                "status_counts": dict(sorted(Counter(row["status"] for row in self.attempts).items())),
            },
        }
    ...
    def __post_init__(self) -> None:
        ...
        if hash_json(logical_payload) != self.attempt_store_logical_state_sha256:
            raise ChildSnapshotIntegrityError("child attempt logical-state hash mismatch")
        ...
        if metadata.get("workspace_descriptor_hash") != self.workspace_descriptor_hash:
            raise ChildSnapshotIntegrityError("attempt metadata descriptor hash mismatch")
    ...
    def from_core_dict(cls, value: Mapping[str, Any]) -> "ChildAttemptSnapshot":
        ...
        if set(value) != expected or value.get("schema_version") != CHILD_ATTEMPT_SNAPSHOT_SCHEMA:
        ...
        checkpoints = tuple(dict(row) for row in attempt_store["evaluation_checkpoints"])
...
def import_child_workspace_snapshot(
    ...
    *,
    ...
    try:
        ...
        raise ChildSnapshotIntegrityError(f"cannot verify child manifest: {exc}") from exc
    ...
    core = {
        ...
        "attempt_store": {
            ...
            "status_counts": dict(
                sorted(Counter(row["status"] for row in logical["evaluation_attempts"]).items())
            ),
        },
    }
...
class CampaignLedgerSealReceipt:
    ...
    def build(
        ...
        *,
        ...
        return cls(**{key: value for key, value in core.items() if key != "schema_version"}, content_sha256=hash_json(core))
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "CampaignLedgerSealReceipt":
        ...
        if not isinstance(value, Mapping) or set(value) != expected:
            raise CampaignAttemptConflict("campaign ledger seal receipt schema is invalid")
...
def _checkpoint_rows_for(snapshot: ChildAttemptSnapshot, attempt_id: int) -> list[dict[str, Any]]:
    return [dict(row) for row in snapshot.checkpoints if row["attempt_id"] == attempt_id]
...
def _validate_attempt_evolution(
    ...
    for name in _ATTEMPT_IMMUTABLE_FIELDS:
        if old_attempt.get(name) != new_attempt.get(name):
            raise ChildSnapshotIntegrityError(f"child attempt immutable field changed: {name}")
    old_status = old_attempt["status"]
    new_status = new_attempt["status"]
    if old_status in _TERMINAL_CHILD_STATUSES:
        if dict(old_attempt) != dict(new_attempt) or [dict(row) for row in old_checkpoints] != [
...
class CampaignAttemptLedger:
    ...
    def _transaction(self) -> Iterator[sqlite3.Connection]:
        if self.connection.in_transaction:
            raise CampaignAttemptLedgerError("nested ledger transactions are not supported")
        try:
    ...
    def _initialize(self, requested_parent_hash: str | None) -> None:
        ...
        existing = dict(self.connection.execute("SELECT key,value FROM ledger_metadata"))
        if not existing:
            values = {
            ...
            self.connection.executemany(
                "INSERT INTO ledger_metadata(key,value) VALUES (?,?)", sorted(values.items())
            )
            self.connection.commit()
        ...
        if any(existing.get(name) != value for name, value in required.items()):
            raise CampaignAttemptLedgerError("ledger binding differs from requested campaign")
        stored_parent = existing.get("parent_checkpoint_hash", "") or None
        lifecycle = existing.get("lifecycle_state", "open")
        if lifecycle == "sealed":
            if requested_parent_hash is not None and requested_parent_hash != stored_parent:
        ...
        if requested_parent_hash is not None and stored_parent not in (None, requested_parent_hash):
    ...
    def _lifecycle_locked(self, connection: sqlite3.Connection) -> str:
        ...
        if row is None or row[0] not in {"open", "sealed"}:
            raise CampaignAttemptLedgerError("campaign ledger lifecycle metadata is invalid")
        return str(row[0])
    ...
    def _consumed_locked(self, connection: sqlite3.Connection) -> int:
        reservations = int(connection.execute("SELECT COUNT(*) FROM campaign_reservations").fetchone()[0])
        unbound = int(
            connection.execute(
    ...
    def _assert_capacity_locked(self, connection: sqlite3.Connection, increment: int = 1) -> None:
    ...
    def _reservation_from_row(self, row: sqlite3.Row) -> CampaignReservation:
        return CampaignReservation(
            ...
            child_attempt_id=(None if row["child_attempt_id"] is None else int(row["child_attempt_id"])),
    ...
    def reserve(
        ...
        *,
        ...
        binding = (candidate, generation, run_id, worker, input_digest, evaluator_digest)
        with self._transaction() as connection:
            ...
            if existing is not None:
                ...
                cursor = connection.execute(
                    ...
                    (key, candidate, generation, run_id, worker, input_digest, evaluator_digest, timestamp, timestamp),
                )
                attempt_id = int(cursor.lastrowid)
        row = self.connection.execute(
            "SELECT * FROM campaign_reservations WHERE global_attempt_id=?", (attempt_id,)
        ).fetchone()
        return self._reservation_from_row(row)
    ...
    def mark_unattached_terminal(
        ...
        *,
        ...
        if status.value not in {
            ...
            raise ValueError("unattached terminal status must consume a failed-like outcome")
        key = _require_code(idempotency_key, "idempotency_key")
        ...
        with self._transaction() as connection:
            ...
            if row["child_attempt_id"] is not None:
                raise CampaignAttemptConflict("attached reservation status comes from child snapshot")
            if row["status"] in _TERMINAL_LEDGER_STATUSES:
                if row["status"] != status.value or row["failure_code"] != code:
                    raise CampaignAttemptConflict("terminal campaign reservation is immutable")
            else:
                connection.execute(
                    """UPDATE campaign_reservations SET status=?,failure_code=?,updated_at=?
                       WHERE global_attempt_id=?""",
                    (status.value, code, _iso(now or _utc_now()), row["global_attempt_id"]),
                )
        return self.get_reservation(key)

    def _bind_parent_checkpoint_locked(self, connection: sqlite3.Connection, digest: str) -> None:
        stored = connection.execute(
            "SELECT value FROM ledger_metadata WHERE key='parent_checkpoint_hash'"
        ).fetchone()[0]
        if stored and stored != digest:
            raise CampaignAttemptConflict("campaign ledger is bound to another parent checkpoint")
        if not stored:
            connection.execute(
                "UPDATE ledger_metadata SET value=? WHERE key='parent_checkpoint_hash'", (digest,)
            )
    ...
    def _attach_locked(
        ...
        *,
        ...
        reservation = connection.execute(
            "SELECT * FROM campaign_reservations WHERE idempotency_key=?", (idempotency_key,)
        ).fetchone()
        ...
        if child is None:
            raise CampaignAttemptConflict("child attempt is not present in imported snapshot")
        expected = (
        ...
        actual = (child["run_id"], child["candidate_id"], child["input_hash"], child["evaluator_hash"])
        if actual != expected:
            raise CampaignAttemptConflict("child attempt does not match global reservation binding")
        ...
        if existing_reservation_id is not None and int(existing_reservation_id) != int(
            ...
            raise CampaignAttemptConflict("child attempt is attached to another reservation")
        old_child_key = (
        ...
        if old_child_key != (None, None) and old_child_key != new_child_key:
            raise CampaignAttemptConflict("global reservation is attached to another child attempt")
        changed = existing_reservation_id is None or old_child_key == (None, None)
        connection.execute(
            """UPDATE child_attempts SET reservation_id=?
               WHERE workspace_descriptor_hash=? AND child_attempt_id=?""",
            (reservation["global_attempt_id"], workspace_descriptor_hash, child_attempt_id),
        )
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            member = connection.execute(
                ...
                (snapshot_hash, snapshot["workspace_descriptor_hash"], child_attempt_id),
            ).fetchone()
            if member is None:
                raise CampaignAttemptConflict("child attempt is not a member of the named snapshot")
            self._attach_locked(
                connection,
                idempotency_key=key,
    ...
    def _reconcile_locked(
        ...
        *,
        ...
        if snapshot.protocol_hash != self.protocol_hash:
            raise ChildSnapshotIntegrityError("child snapshot protocol differs from campaign")
        ...
        if existing_snapshot is not None and existing_snapshot["snapshot_json"] != snapshot_json:
            raise ChildSnapshotIntegrityError("snapshot hash collision or stored snapshot tampering")
        ...
        if not existing_ids.issubset(snapshot_ids):
            raise ChildSnapshotIntegrityError("new child snapshot truncates prior attempt history")
        ...
        for attempt in snapshot.attempts:
            ...
            if existing is None:
                ...
                _validate_attempt_evolution(old_attempt, old_checkpoints, attempt, checkpoint_rows)
        ...
        if self._consumed_locked(connection) > self.max_attempts:
            raise CampaignAttemptBudgetExceeded("child reconciliation would exceed campaign budget")
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            for snapshot in checkpoint.child_snapshots:
                intermediate.append(
                    (snapshot, self._reconcile_locked(connection, snapshot, {}, now=timestamp))
                )
            imported_keys = {
            ...
            if len(imported_keys) != checkpoint.consumed_attempts:
                raise CampaignAttemptConflict("parent checkpoint unique attempt count mismatch")
    ...
    def usage(self) -> CampaignAttemptUsage:
        ...
        global_count = int(
            self.connection.execute("SELECT COUNT(*) FROM campaign_reservations").fetchone()[0]
        )
        unbound = int(
            self.connection.execute(
                "SELECT COUNT(*) FROM child_attempts WHERE reservation_id IS NULL"
        ...
        snapshots = int(self.connection.execute("SELECT COUNT(*) FROM child_snapshots").fetchone()[0])
    ...
    def _assert_sealable_locked(self, connection: sqlite3.Connection) -> None:
        ...
        if active_reservations or active_children:
            raise CampaignAttemptConflict("campaign ledger has active or nonterminal attempts")
        ...
        for row in reservations:
            ...
            if child is None or child["reservation_id"] != row["global_attempt_id"]:
                raise CampaignAttemptConflict("reservation/child binding is not symmetric")
            attempt = json.loads(child["attempt_json"])
            expected_status = _child_status(attempt).value
            if row["status"] != expected_status:
                raise CampaignAttemptConflict("reservation status differs from child attempt")
        ...
        for row in children:
            ...
            if row["child_status"] != attempt.get("status"):
                raise CampaignAttemptConflict("child_status differs from attempt_json.status")
            member = connection.execute(
            ...
            if member is None:
                raise CampaignAttemptConflict("final child snapshot membership is missing")
            if member["attempt_sha256"] != hash_json(attempt) or member[
                "checkpoints_sha256"
            ] != hash_json(checkpoints):
                raise CampaignAttemptConflict("final child snapshot membership hash mismatch")
    ...
    def seal_and_close(
        ...
        *,
        ...
        if checkpoint is None or int(checkpoint[0]) != 0:
            raise CampaignAttemptLedgerError("cannot checkpoint campaign ledger before seal")
        self.close()
...
def load_campaign_ledger_seal_receipt(
    ...
    try:
        ...
        raise CampaignAttemptConflict("cannot load campaign ledger seal receipt") from exc
...
def _readonly_ledger_snapshot(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    try:
        ...
        raise CampaignAttemptConflict("sealed ledger maximum attempt metadata is invalid") from exc
...
def load_sealed_campaign_ledger_snapshot(
    ...
    try:
        ...
        if "connection" in locals():
            connection.close()
    if integrity != "ok":
        raise CampaignAttemptConflict(f"sealed campaign ledger integrity failed: {integrity}")
    metadata = {
    ...
    if any(metadata.get(key) != value for key, value in required.items()):
        raise CampaignAttemptConflict("sealed campaign ledger metadata differs from receipt")
...
class CampaignAttemptCheckpoint:
    ...
    def _core_dict(self) -> dict[str, Any]:
        return {
            ...
            "child_snapshots": [snapshot.to_dict() for snapshot in self.child_snapshots],
        }
    ...
    def __post_init__(self) -> None:
        ...
        if self.consumed_attempts + self.remaining_attempts != self.max_attempts:
            raise CampaignAttemptConflict("checkpoint budget arithmetic is inconsistent")
        identities = {
        ...
        if len(identities) != self.consumed_attempts:
            raise CampaignAttemptConflict("checkpoint consumed count differs from unique attempts")
        required_registry = {
            "registry_sha256",
        ...
        if set(self.parent_registry) != required_registry:
            raise CampaignAttemptConflict("parent registry checkpoint schema is invalid")
        ids = self.parent_registry["candidate_ids"]
        if not isinstance(ids, list) or ids != sorted(set(ids)):
            raise CampaignAttemptConflict("parent candidate ids must be sorted and unique")
        ...
        if hash_json(self._core_dict()) != self.content_sha256:
            raise CampaignAttemptConflict("campaign attempt checkpoint content hash mismatch")
        if self.file_sha256 is not None:
    ...
    def from_dict(
        ...
        if not isinstance(value, Mapping) or set(value) != expected:
            raise CampaignAttemptConflict("campaign attempt checkpoint schema is invalid")
        if value["schema_version"] != CAMPAIGN_ATTEMPT_CHECKPOINT_SCHEMA:
            raise CampaignAttemptConflict("campaign attempt checkpoint version is unsupported")
        return cls(
            ...
            child_snapshots=tuple(
                ChildAttemptSnapshot.from_dict(item) for item in value["child_snapshots"]
            ),
...
def load_campaign_attempt_checkpoint(path: str | Path) -> CampaignAttemptCheckpoint:
    ...
    try:
        ...
        raise CampaignAttemptConflict(f"cannot load campaign attempt checkpoint: {exc}") from exc
    if expected != actual:
