...
class ParetoObjectiveSpec:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise ParetoGovernanceError("unknown Pareto objective or direction") from exc
...
def _validated_specs(
    ...
    if len(names) != len(expected) or set(names) != expected:
        ...
        duplicates = sorted(
            {item.value for item in names if names.count(item) > 1}
        )
        raise ParetoGovernanceError(
...
class ParetoObservation:
    ...
    def value(self, objective: ParetoObjective | str) -> float:
        name = objective.value if isinstance(objective, ParetoObjective) else str(objective)
...
class ParetoRoundObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if len(ids) != len(set(ids)):
            raise ParetoGovernanceError("round contains duplicate candidate observations")
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "ParetoRoundObservation":
        ...
        if any(not isinstance(item, Mapping) for item in raw):
            raise ParetoGovernanceError(
                "round observation records must be objects"
            )
        return cls(
            campaign_round=value["campaign_round"],
...
def epsilon_dominates(
    ...
    for spec in specs:
        ...
        if spec.direction is ObjectiveDirection.MAXIMIZE:
            ...
            meaningfully_better = (
                meaningfully_better
                or left_value > right_value + spec.epsilon
            )
        else:
            no_worse = no_worse and left_value <= right_value + spec.epsilon
            meaningfully_better = (
...
def pareto_frontier(
    ...
    for item in items:
        if not isinstance(item, ParetoObservation):
            raise ParetoGovernanceError("frontier input contains an untyped observation")
    ...
    return tuple(sorted(frontier, key=lambda item: (item.campaign_round, item.candidate_id)))
...
def build_campaign_frontier(
    observations: (
        Iterable[ParetoObservation | ParetoRoundObservation]
...
def frontier_progressed(
    ...
    if current.objective_spec_hash != expected_hash or (
        ...
        raise ParetoGovernanceError(
            "frontier objective specification hash mismatch"
        )
    ...
    for candidate in current.members:
        ...
        if any(epsilon_equivalent(candidate, old, specs) for old in previous.members):
            continue
        if not any(epsilon_dominates(old, candidate, specs) for old in previous.members):
            return True
    return False
...
def select_diverse_shortlist(
    ...
    *,
    ...
    correlation_limit = _finite_number(
    ...
    while remaining and len(selected) < capacity:
        ...
        for spec in specs:
            candidate = next(
                (
                    item
                    for item in queues[spec.name]
                    if item.candidate_id in remaining
                ),
                None,
            )
...
class ParetoObservationLedger:
    ...
    def __init__(self, entries: Sequence[ParetoLedgerEntry] = ()) -> None:
        ...
        for entry in entries:
            if not isinstance(entry, ParetoLedgerEntry):
                raise ParetoGovernanceError(
                    "Pareto ledger contains an untyped entry"
                )
    ...
    def append(self, observation: ParetoRoundObservation) -> ParetoLedgerEntry:
        ...
        entry = ParetoLedgerEntry.create(
            len(self._entries), observation, previous_hash
        )
        self._append_verified(entry)
        return entry
    ...
    def _append_verified(self, entry: ParetoLedgerEntry) -> None:
        ...
        if self.last_campaign_round is not None and campaign_round <= self.last_campaign_round:
            raise ParetoGovernanceError(
                "Pareto ledger rounds must be strictly increasing"
            )
        new_ids = {
            item.candidate_id for item in entry.round_observation.observations
        }
...
class ParetoStoppingDecision:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.should_stop, bool) or not isinstance(
            ...
            raise ParetoGovernanceError(
                "stopping flags must be booleans"
            )
...
class ParetoCheckpoint:
    ...
    def __post_init__(self) -> None:
        ...
        if reason is StopReason.CONTINUE and self.stalled_rounds >= self.patience_rounds:
            raise ParetoGovernanceError(
                "continue checkpoint exceeds Pareto patience"
            )
        if (
