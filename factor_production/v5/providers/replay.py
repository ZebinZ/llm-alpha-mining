...
class ReplayProvider(CandidateProvider):
    ...
    @property
    def candidates(self) -> tuple[CandidateSpecV5, ...]:
        """Immutable replay contents for fail-closed campaign preflight."""

        return self._candidates

    def _load(self) -> tuple[CandidateSpecV5, ...]:
        ...
        for line_number, raw_line in enumerate(self.path.read_text(encoding="utf-8").splitlines(), 1):
            ...
            try:
                ...
                spec = CandidateSpecV5.from_dict(spec_payload)
            except Exception as exc:
                raise ReplayError(f"invalid replay record at {self.path}:{line_number}: {exc}") from exc
            ...
            if spec.candidate_id in ids:
                raise ReplayError(f"duplicate candidate id at {self.path}:{line_number}: {spec.candidate_id}")
    ...
    def propose(self, context: ProposalContext) -> ProposalBatch:
        ...
        return ProposalBatch(self.name, selected, exhausted=len(selected) == len(matching))
