...
from factor_production.v5.domain.enums import ProposalKind
from factor_production.v5.domain.models import CandidateSpecV5
from factor_production.v5.llm.panel import AlphaResearchPanel, PanelResult
from factor_production.v5.llm.safe_context import (
    ParentCatalogEntry,
    SafeResearchContext,
    UnsafeLLMContext,
)
from factor_production.v5.protocol import V5Protocol
...
class LLMPanelProvider(CandidateProvider):
    ...
    def __init__(
        ...
        *,
        ...
        parent_catalog_resolver: Callable[[ProposalContext], Iterable[ParentCatalogEntry]] | None = None,
    ) -> None:
        ...
        if parent_catalog and parent_catalog_resolver is not None:
            raise ValueError("configure a static parent_catalog or a resolver, not both")
        self._parent_catalog = tuple(parent_catalog)
        self._parent_catalog_resolver = parent_catalog_resolver
        if any(not isinstance(item, ParentCatalogEntry) for item in self._parent_catalog):
            raise TypeError("parent_catalog accepts ParentCatalogEntry only")
        if len({item.spec_hash for item in self._parent_catalog}) != len(self._parent_catalog):
    ...
    def _safe_context(self, context: ProposalContext) -> SafeResearchContext:
        if context.protocol_hash != self.protocol.content_hash:
            raise UnsafeLLMContext("proposal context is not bound to the configured protocol")
        ...
        ordered_catalog = tuple(by_hash[digest] for digest in context.prior_candidate_hashes)
    ...
    def propose(self, context: ProposalContext) -> ProposalBatch:
        ...
        for draft in result.selected:
            if context.generation == 0:
                ...
                try:
                    ...
                    raise UnsafeLLMContext("selected draft has an unknown parent") from None
            else:
        ...
        return ProposalBatch(provider=self.name, candidates=tuple(specs), exhausted=False)
