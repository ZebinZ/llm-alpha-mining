...
from factor_eval_minute import operators as dataframe_operators
...
class SafeExpressionInterpreter:
    ...
    def validate(
        ...
        *,
        ...
        if depth > self.max_call_depth:
            reasons.append(
                f"max_call_depth_exceeded:{depth}>{self.max_call_depth}"
            )
        for field in fields:
    ...
    def evaluate(
        ...
        if not validation.is_valid:
            raise ValueError("invalid_factor_expression:" + ";".join(validation.reasons))
        tree = ast.parse(str(expression), mode="eval")
        ...
        if isinstance(node, ast.Call):
            ...
            if spec is None:
                ...
                if spec.window_argument is not None and len(node.args) > spec.window_argument:
                    ...
                    if not isinstance(window_node, ast.Constant) or isinstance(
                        window_node.value, bool
