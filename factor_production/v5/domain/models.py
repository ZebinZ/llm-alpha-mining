...
from factor_production.v5.domain.enums import FactorFrequency, ProposalKind
...
def _deep_freeze(value: Any, *, path: str = "$") -> JsonValue:
    ...
    raise CandidateSpecError(f"value at {path} is not JSON-compatible: {type(value).__name__}")
...
def _strict_string_tuple(value: Any, name: str, *, required: bool = False) -> tuple[str, ...]:
...
class CandidateSpecV5:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "protocol_hash", _nonempty(self.protocol_hash, "protocol_hash"))
        object.__setattr__(self, "provider", _nonempty(self.provider, "provider"))
        try:
            frequency = FactorFrequency(self.frequency)
        except ValueError as exc:
            raise CandidateSpecError(f"unsupported frequency: {self.frequency!r}") from exc
        ...
        required_fields = _strict_string_tuple(self.required_fields, "required_fields", required=True)
        ...
        if explicit_multi_generation:
            ...
            if (
                ...
                raise CandidateSpecError("campaign_round must be a non-negative integer")
            if (
            ...
            if self.generation == 0 and parent_ids:
                raise CandidateSpecError("generation zero candidate cannot have parents")
            if self.generation > 0 and not parent_ids:
                raise CandidateSpecError("non-zero generation candidate must name at least one parent")
        ...
        object.__setattr__(self, "_explicit_multi_generation", explicit_multi_generation)
        ...
        object.__setattr__(self, "aggregation", _deep_freeze(self.aggregation, path="$.aggregation"))
        object.__setattr__(self, "parameters", _deep_freeze(self.parameters, path="$.parameters"))
        ...
        *,
        ...
        if proposal_kind is ProposalKind.ROOT:
            if parent_count or lineage_depth != 0:
                raise CandidateSpecError("root proposals require no parents and lineage_depth zero")
            return
        if lineage_depth <= 0:
        ...
        if proposal_kind in {ProposalKind.MUTATION, ProposalKind.REPAIR}:
            ...
            return
        if proposal_kind in {ProposalKind.CROSSOVER, ProposalKind.COMPOSITE} and parent_count < 2:
            raise CandidateSpecError(
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, Any]) -> "CandidateSpecV5":
        allowed = {
            "schema_version",
            "candidate_id",
            "hypothesis",
        ...
        if unknown:
            raise CandidateSpecError(f"unknown CandidateSpecV5 fields: {unknown}")
        required = allowed - {
            "schema_version",
            "parent_ids",
            "aggregation",
        ...
        if missing:
            raise CandidateSpecError(f"missing CandidateSpecV5 fields: {missing}")
        return cls(
