    ...
    *,
    ...
    for signal_date, next_signal_date in zip(signals[:-1], signals[1:], strict=True):
        ...
        if entry_position >= len(calendar) or exit_position >= len(calendar):
            raise ValueError(
                "execution_boundary_outside_calendar:"
                f"{signal_date}:{next_signal_date}"
            )
...
def compute_execution_aware_weekly_returns(
    ...
    *,
    ...
    required_dates = list(
        dict.fromkeys(
            windows["entry_date"].tolist() + windows["exit_date"].tolist()
        )
    )
    ...
    for window in windows.itertuples(index=False):
        ...
        exit_adjustment_invalid = ~np.isfinite(exit_adjustment) | (
            exit_adjustment <= 0
        )
        ...
        diagnostics.append(
            {
                ...
                "entry_amount_invalid_count": int(
                    (scope & entry_amount_invalid).sum()
                ),
...
def select_execution_labels_between(
    ...
    selected = label_windows[
        (label_windows["entry_date"] >= start)
        & (label_windows["exit_date"] <= end)
    ]
