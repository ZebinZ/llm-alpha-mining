...
class ForwardReturnPolicy:
    ...
    def __post_init__(self) -> None:
        if self.missing_observation != "invalidate_window":
            raise ValueError(
                "V5 only supports fail-closed unexplained missing returns"
            )
    ...
    *,
    ...
    for window in label_windows.itertuples(index=False):
        ...
        if start not in returns.index or end not in returns.index:
            raise ValueError(f"label_window_outside_calendar:{signal_date}:{start}:{end}")
        ...
        compounded = (1.0 + window_returns).prod(axis=0, min_count=len(window_returns)) - 1.0
