...
from factor_production.v5.llm.safe_context import UnsafeLLMContext
from factor_production.v5.llm.transport.base import StructuredTransport, TransportError
...
class ReplayEntry:
    ...
    def _to_response(self) -> StructuredCallResponse:
        return StructuredCallResponse(
            ...
            provider_request_id=validate_provider_request_digest(self.provider_request_id),
        )
    ...
    def from_dict(
        ...
        *,
        ...
        if value["request_hash"] != request.request_hash or value["role"] != request.role.value:
        ...
        try:
            ...
            raise ValueError("persisted replay provider request ID must be hashed") from None
        entry = cls(
            request=request,
            output=value["output"],
    ...
    *,
    ...
    for line_number, line in enumerate(source.read_text(encoding="utf-8").splitlines(), 1):
        ...
        try:
            ...
            request_hash = payload.get("request_hash") if isinstance(payload, Mapping) else None
            request = frozen_requests.get(request_hash) if isinstance(request_hash, str) else None
