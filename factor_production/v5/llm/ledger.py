...
from factor_production.v5.llm.domain import StructuredCallRequest, StructuredCallResponse, Usage

if TYPE_CHECKING:
    from factor_production.v5.llm.budget import BudgetSnapshot, LLMBudgetLimits
...
class LedgerPin:
    ...
    def __post_init__(self) -> None:
        if _DIGEST.fullmatch(self.head_hash) is None or _DIGEST.fullmatch(self.file_hash) is None:
            raise LedgerIntegrityError("ledger pin hashes must be lowercase SHA-256 digests")
...
def _validate_nonnegative_int_mapping(value: Any, fields: set[str], name: str) -> dict[str, int]:
...
class CallLedger:
        ...
        *,
        ...
        if self._records:
            if expected_head_hash is None or expected_file_hash is None:
                raise LedgerIntegrityError("external expected ledger head and file hash are required")
            ...
            if _file_hash(self.path) != expected_file_hash:
                raise LedgerIntegrityError("external expected ledger file hash mismatch")
        elif expected_head_hash not in {None, GENESIS_HASH}:
            raise LedgerIntegrityError("empty ledger does not match expected head")
        elif expected_file_hash is not None and _file_hash(self.path) != expected_file_hash:
    ...
    def _read_handle(self, handle: Any) -> list[dict[str, Any]]:
        ...
        for line_number, line in enumerate(handle.read().splitlines(), 1):
            ...
            try:
                ...
                raise LedgerIntegrityError(f"invalid ledger JSON at line {line_number}") from None
            expected = {
                "schema_version",
                "seq",
                "created_ns",
            ...
            if not isinstance(record, dict) or set(record) != expected:
                raise LedgerIntegrityError(f"invalid ledger fields at line {line_number}")
            supplied_hash = record["record_hash"]
            body = {key: value for key, value in record.items() if key != "record_hash"}
            if record["schema_version"] != LEDGER_SCHEMA:
                raise LedgerIntegrityError(f"unsupported ledger schema at line {line_number}")
            if record["seq"] != len(records) + 1:
                raise LedgerIntegrityError(f"non-contiguous ledger sequence at line {line_number}")
            ...
            if supplied_hash != hash_json(body):
                raise LedgerIntegrityError(f"ledger record hash mismatch at line {line_number}")
            if _DIGEST.fullmatch(str(record["request_hash"])) is None:
                raise LedgerIntegrityError(f"invalid request hash at line {line_number}")
    ...
    def _validate_state_machine(records: list[dict[str, Any]]) -> None:
        ...
        for record in records:
            ...
            if state is not None:
                if state["request_hash"] != record["request_hash"] or state["role"] != record["role"]:
                    raise LedgerIntegrityError("operation identity changed inside ledger")
            event = record["event"]
            ...
            if event == "attempt_started":
                ...
                if set(details) != expected:
                    raise LedgerIntegrityError("attempt_started details fields must be exact")
                reservation = _validate_nonnegative_int_mapping(details["reservation"], _USAGE_FIELDS, "reservation")
                limits = _validate_nonnegative_int_mapping(details["budget_limits"], _LIMIT_FIELDS, "budget limits")
                ...
                if state is None:
                    if attempt != 1:
                        raise LedgerIntegrityError("new operation must start at attempt one")
                ...
                continue
            if state is None or state["phase"] != "active" or attempt != state["attempt"]:
            ...
            if event == "call_succeeded":
                if set(details) != {"charge", "response"}:
                    raise LedgerIntegrityError("call_succeeded details fields must be exact")
                response = StructuredCallResponse.from_dict(details["response"])
                if response.request_hash != record["request_hash"]:
                    raise LedgerIntegrityError("persisted response request hash mismatch")
                charge = _validate_nonnegative_int_mapping(details["charge"], _USAGE_FIELDS, "charge")
                if charge != response.usage.to_dict():
                    raise LedgerIntegrityError("success charge differs from response usage")
                if any(charge[name] > state["reservation"][name] for name in _USAGE_FIELDS):
                ...
                if response_hash is not None and _DIGEST.fullmatch(str(response_hash)) is None:
                ...
                if event == "attempt_retryable_failure" and charge != _zero_usage():
                    raise LedgerIntegrityError("definitely-unsent retryable failure must have zero charge")
                if event == "call_indeterminate" and charge != state["reservation"]:
                    raise LedgerIntegrityError("indeterminate call must conservatively charge reservation")
                phase = {
            ...
            with self.path.open("rb") as handle:
                ...
                try:
                    ...
                    try:
                        ...
                        raise LedgerIntegrityError("ledger is not valid UTF-8") from None
    ...
    @staticmethod
    def _operation_from_records(records: list[dict[str, Any]], operation_id: str) -> LedgerOperation | None:
    ...
    @staticmethod
    def _enforce_budget(records: list[dict[str, Any]], limits: "LLMBudgetLimits") -> None:
        from factor_production.v5.llm.budget import LLMBudgetExceeded
        ...
        exceeded = [name for name, (actual, maximum) in checks.items() if actual > maximum]
        if exceeded:
            raise LLMBudgetExceeded("LLM budget exceeded: " + ",".join(exceeded))

    def budget_snapshot(self, limits: "LLMBudgetLimits") -> "BudgetSnapshot":
        from factor_production.v5.llm.budget import BudgetSnapshot

        self._reload()
        calls, input_tokens, output_tokens, cost, pending = self._budget_values(self._records)
    ...
    def reserve_and_start(
        ...
        *,
        ...
        reservation_dict = _validate_nonnegative_int_mapping(reservation, _USAGE_FIELDS, "reservation")
            ...
            try:
                ...
                if existing is not None:
                    if existing.request_hash != request.request_hash:
                        raise LedgerIntegrityError("operation_id reuse with a different request")
                    if existing.state != "retryable_failure" or attempt != existing.next_attempt:
                        raise IndeterminateCallError(
                            f"operation {request.operation_id} is {existing.state}; redispatch denied"
                        )
                elif attempt != 1:
                    raise LedgerIntegrityError("a new operation must begin at attempt one")
                details = {
    ...
    def _finalize(
        ...
        *,
        ...
        if event == "call_succeeded":
            ...
            details: dict[str, Any] = {"charge": charge.to_dict(), "response": response.to_dict()}
        else:
            if (
                ...
                raise LedgerIntegrityError("terminal failure requires a closed safe error code")
    ...
    def retryable_failure(self, request: StructuredCallRequest, *, attempt: int, error_code: str) -> None:
        self._finalize(
            request,
            attempt=attempt,
    ...
    def indeterminate(self, request: StructuredCallRequest, *, attempt: int, error_code: str) -> None:
        self._reload()
        rows = [
        ...
        if not rows:
            raise LedgerIntegrityError("indeterminate finalization lacks active reservation")
    ...
    def success(
        ...
        *,
        ...
        if response.model_id not in set(allowed_model_ids):
            raise LedgerIntegrityError("response model is outside the closed allow-list")
        ...
        if request.role.value == "proposer":
            ...
            try:
                ...
                raise LedgerIntegrityError("review descriptor binding is malformed") from None
            if (
