...
from factor_production.v5.dsl.interpreter import OperatorRegistry, SafeExpressionInterpreter
...
from factor_production.v5.llm.safe_context import SafeResearchContext, assert_safe_context
from factor_production.v5.llm.schemas import CandidateDraft, Review, StructuredOutputError
...
class AlphaResearchPanel:
    ...
    def __init__(self, executor: StructuredCallExecutor, *, max_expression_depth: int = 6) -> None:
    ...
    @staticmethod
    def _require_exact_coverage(role: str, reviews: tuple[Review, ...], ids: set[str]) -> None:
    ...
    def _preflight(
        ...
        if invalid_windows:
            reasons.append("windows_not_allowed:" + ",".join(map(str, sorted(invalid_windows))))
    ...
    def run(self, context: SafeResearchContext) -> PanelResult:
        ...
        for draft in proposed:
            ...
            if draft.semantic_hash in seen_semantics or draft.semantic_hash in prior_semantics:
