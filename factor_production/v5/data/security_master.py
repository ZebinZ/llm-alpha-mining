...
from factor_production.v5.data.universe import universe_mask_from_fast_cache
...
def build_clean_stock_universe(
    ...
    *,
    ...
    excluded_399 = sorted(
        code for code in excluded if str(code).startswith("399")
    )
...
def _normalize_axes(frame: pd.DataFrame, *, source_name: str) -> pd.DataFrame:
    ...
    out.index = pd.Index(
        [_normalize_date(item) for item in out.index], name="date"
    )
    ...
    if not out.columns.is_unique:
        raise ValueError(
            f"{source_name} securities are not unique after normalization"
        )
