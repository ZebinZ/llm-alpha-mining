...
_TRUE_SUSPENSION_MARKERS = frozenset(
    {"1", "S", "SUSPENDED", "SUSPEND", "停牌", "TRUE"}
)
...
def security_data_contract_hash(contract: SecurityDataContract) -> str:
    ...
    payload = {
        ...
        "signal_universe_mask": _hash_contract_frame(
            contract.signal_universe_mask
        ),
        "execution_tradability_mask": _hash_contract_frame(
            contract.execution_tradability_mask
        ),
        "suspension_marker_mask": _hash_contract_frame(
            contract.suspension_marker_mask
        ),
...
def apply_feature_return_policy(
    ...
    *,
    ...
    confirmed_suspension = (
        identity & marker & values.isna() & ~positive_amount
    )
    ...
    *,
    ...
    for column_position, instrument_id in enumerate(securities):
        ...
        ordered = group.sort_values(
            ["effective_date", "known_at"], kind="mergesort"
        )
...
def _research_identity_from_frdata(
    ...
    *,
    ...
    for axis in axes:
        evidence_ids &= axis
    evidence_ids = {
        item for item in evidence_ids if len(item) == 6 and item.isdigit()
    }
...
def _assemble_contract(
    ...
    *,
    ...
    signal_source, signal_missing_dates, signal_missing_securities = (
        _align_daily_bool(
    ...
    audit = SecurityDataContractAudit(
        ...
        suspension_trade_conflict_cell_count=_true_count(
            suspension_trade_conflict
        ),
...
def _normalize_instrument_events(events: pd.DataFrame) -> pd.DataFrame:
    ...
    frame["instrument_id"] = frame["instrument_id"].map(
    ...
    if invalid_status:
        raise ValueError("instrument events contain invalid status:" + ",".join(invalid_status))
    if frame.duplicated(
...
def _normalize_listing_dates(
    ...
    if not values.index.is_unique:
        raise ValueError("listing-date instrument ids are not unique after normalization")
