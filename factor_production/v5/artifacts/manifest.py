...
def _safe_relative_path(value: str) -> str:
    ...
    if path.is_absolute() or ".." in path.parts or not path.parts:
        raise ArtifactError(f"artifact location must be a safe relative path: {value!r}")
...
class ArtifactManifest:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "artifacts": [record.to_dict() for record in sorted(self.artifacts, key=lambda item: item.logical_name)],
    ...
    def write(self, path: str | Path, *, write_sidecar: bool = True) -> Path:
        ...
        if write_sidecar:
            ...
            _atomic_write(destination.with_suffix(destination.suffix + ".sha256"), f"{digest}  {destination.name}\n".encode())
        return destination

    @classmethod
    def load(cls, path: str | Path, *, verify_sidecar: bool = True) -> "ArtifactManifest":
        source = Path(path)
        if verify_sidecar:
            ...
            if actual != expected:
                raise ArtifactError(f"manifest hash mismatch: expected {expected}, got {actual}")
    ...
    def verify(self, root: str | Path) -> ManifestVerification:
        ...
        for record in self.artifacts:
            ...
            if size != record.size_bytes:
                errors.append(f"size_mismatch:{record.logical_name}:{record.size_bytes}!={size}")
            digest = hash_file(path)
            if digest != record.sha256:
                errors.append(f"hash_mismatch:{record.logical_name}:{record.sha256}!={digest}")
        return ManifestVerification(ok=not errors, checked=len(self.artifacts), errors=tuple(errors))


class ArtifactStore:
    """Immutable content-addressed object store rooted inside a run workspace."""

    def __init__(self, workspace_root: str | Path, *, object_dir: str = "artifacts/objects") -> None:
    ...
    def read_bytes(self, record: ArtifactRecord) -> bytes:
        ...
        path = self.workspace_root.joinpath(
            *PurePosixPath(record.location).parts
        )
    ...
    def put_json(self, logical_name: str, value: Any, *, role: str = "research") -> ArtifactRecord:
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            raise ArtifactError(
                f"artifact staging object is incomplete: {staging}"
            )
...
def _verify_regular_file(
    ...
    *,
    ...
    try:
        ...
        raise ArtifactError(f"artifact object cannot be opened safely: {path}") from error
    try:
        opened = os.fstat(descriptor)
...
def _reject_symlink_components(path: Path, *, root: Path) -> None:
    try:
        ...
        raise ArtifactError(f"artifact path resolves outside workspace: {path}") from error
    current = path
    while True:
        try:
            ...
            raise ArtifactError(f"artifact path cannot be inspected: {current}") from error
