...
[English](reproducibility.md) | **简体中文** · [返回首页](../README.zh-CN.md)

## 离线复现

[中文 README](../README.zh-CN.md) 的安装和运行命令是维护入口。相同软件环境与随机种子下，两次新输出目录的 `report.json` 应具有相同 `scientific_signature`。调用账本包含运行时间，因此不要求整个目录逐字节相同。

`requirements-validated.txt` 记录本次本地验收使用的直接依赖版本，并不是包含所有间接依赖和哈希的锁文件。CI 使用 Python 3.12、3.13 和项目声明的兼容依赖范围；发布版本已在这两个 Python 版本上分别通过 251 项测试、演示及哈希验证；最新状态见 [GitHub Actions](https://github.com/ZebinZ/llm-alpha-mining/actions/workflows/tests.yml)。
