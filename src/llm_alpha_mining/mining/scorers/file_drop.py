...
class FileDropScorer(CandidateScorer):
    ...
    def import_results(
        ...
        *,
        ...
        if result["schema_version"] != DROP_RESULT_SCHEMA:
            raise FileDropError(
                f"unsupported external result schema: {result['schema_version']!r}"
            )
        ...
        for index, item in enumerate(result["results"]):
            ...
            if candidate_id not in expected:
                raise FileDropError(
                    f"unknown external result candidate: {candidate_id}"
                )
            expected_hash, generation = expected[candidate_id]
            if item["spec_hash"] != expected_hash:
                raise FileDropError(
                    f"spec hash mismatch for external result {candidate_id}"
                )
    ...
    def _load_request(self, path: Path) -> dict[str, Any]:
        ...
        core = {
            key: payload[key]
            for key in ("schema_version", "protocol_hash", "scorer", "candidates")
        }
        ...
        if payload["batch_id"] != f"holdout-{actual_request_hash[:20]}":
            raise FileDropError(
                "external request batch ID is not derived from request hash"
            )
        for item in payload["candidates"]:
            if set(item) != {"candidate_id", "spec_hash", "spec"}:
                raise FileDropError("external request candidate record is invalid")
            spec = CandidateSpec.from_dict(item["spec"])
            if (
                item["candidate_id"] != spec.candidate_id
                or item["spec_hash"] != spec.content_hash
            ):
                raise FileDropError(
                    f"external request candidate hash mismatch: {item['candidate_id']}"
                )
        return payload
