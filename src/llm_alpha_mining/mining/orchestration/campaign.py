...
from llm_alpha_mining.mining.domain.enums import CandidateState, RunState, StopReason
...
class CampaignEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.registry = (
            registry if registry is not None else HypothesisRegistry(protocol_hash)
        )
        ...
        if run.protocol_hash != protocol_hash:
            raise CampaignEngineError(
                "repository run protocol does not match campaign protocol"
            )
    ...
    def _synchronize_registry(self) -> None:
        ...
        if not stored:
            if len(self.registry):
                raise CampaignEngineError(
                    "non-empty registry has no matching repository candidates"
                )
            return
        ...
        registry_by_id = {
            entry.candidate_id: entry.spec_hash for entry in self.registry.entries
        }
        if stored_by_id != registry_by_id:
            raise CampaignEngineError(
                "registry and repository candidate ledgers differ"
            )
    ...
    def _assert_active(self) -> None:
        if self._failed:
            raise CampaignEngineError(
                "campaign engine is failed closed after a persistence error"
            )
        if self.should_stop:
    ...
    def propose_round(
        ...
        *,
        ...
        if batch.provider != self.provider.name:
            self._failed = True
            raise CampaignEngineError(
                "proposal batch provider does not match configured provider"
            )
        ...
        accepted_with_entries: list[tuple[CandidateSpec, HypothesisEntry]] = []
        ...
        for candidate in batch.candidates:
            ...
            try:
                ...
                rejected.append(
                    RejectedProposal(candidate.candidate_id, exc.code, exc.detail)
                )
                continue
        ...
        try:
            for candidate, expected_entry in accepted_with_entries:
                ...
                if actual_entry != expected_entry:
                    raise CampaignEngineError(
                        "hypothesis preflight changed during registration"
                    )
        except BaseException:
    ...
    def observe_round(self, observation: ObjectiveObservation) -> StoppingDecision:
        ...
        if self._last_round is None or observation.generation != self._last_round:
            raise CampaignEngineError(
                "objective observation must close the most recently proposed round"
            )
    ...
    def stop_manually(self, detail: str) -> StoppingDecision:
        ...
        if self._stop_decision is None:
            self._stop_decision = StoppingDecision(
                True, StopReason.MANUAL, detail.strip()
            )
        return self._stop_decision
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "stop_reason": (
                self._stop_decision.reason.value
