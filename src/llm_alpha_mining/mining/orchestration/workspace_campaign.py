...
from llm_alpha_mining.mining.artifacts.hashing import hash_file, hash_json
from llm_alpha_mining.mining.domain.enums import CandidateState
from llm_alpha_mining.mining.orchestration.budget import BudgetController, BudgetUsage
from llm_alpha_mining.mining.orchestration.campaign import (
    CampaignConfig,
    CampaignEngine,
)
from llm_alpha_mining.mining.orchestration.hypotheses import HypothesisRegistry
from llm_alpha_mining.mining.orchestration.repository import (
    CandidateRecord,
    EventRecord,
    SQLiteRepository,
)
from llm_alpha_mining.mining.orchestration.stopping import StopController
from llm_alpha_mining.mining.protocol import load_protocol
...
def _load_workspace_descriptor(root: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise WorkspaceCampaignError(
            f"cannot load workspace descriptor {path}: {exc}"
        ) from exc
    expected = {
        "schema_version",
        "run_id",
    ...
    if value["schema_version"] != WORKSPACE_SCHEMA:
        raise WorkspaceCampaignError(
            f"unsupported workspace schema: {value['schema_version']!r}"
        )
    ...
    for name in ("database", "protocol", "manifest"):
        ...
        if location.is_absolute() or ".." in location.parts:
            raise WorkspaceCampaignError(
                f"unsafe workspace {name} path: {value[name]!r}"
            )
    return value
...
def _verify_workspace(root: Path, descriptor: dict[str, str]) -> None:
    ...
    if errors:
        raise WorkspaceCampaignError(
            f"workspace failed immutable verification: {errors}"
        )
    ...
    def from_dict(cls, value: dict[str, object]) -> "CampaignCheckpoint":
        ...
        for name in ("protocol_hash", "registry_hash", "registration_hash"):
            ...
            if (
                ...
                raise WorkspaceCampaignError(
                    f"checkpoint {name} must be a SHA-256 digest"
                )
...
def validate_workspace_branch(
    ...
    with SQLiteRepository(destination_database) as destination_repository:
        destination_hashes = {
            ...
            for record in destination_repository.list_candidates(
                destination_descriptor["run_id"]
            )
        }
    mismatched = {
...
def load_sanitized_feedback_batch(
    ...
    *,
    ...
    try:
        ...
        raise WorkspaceCampaignError(
            f"cannot load sanitized feedback batch: {exc}"
        ) from exc
    expected = {
        "schema_version",
        "campaign_id",
    ...
    if value["visibility"] != "adaptive_local_research_only":
        raise WorkspaceCampaignError(
            "sanitized feedback visibility is not local research"
        )
    if value["contains_raw_metric_values"] is not False:
        raise WorkspaceCampaignError(
            "sanitized feedback batch contains raw metric values"
        )
    if value["contains_external_teacher_feedback"] is not False:
        raise WorkspaceCampaignError(
            "sanitized feedback batch contains external feedback"
        )
    ...
    if not isinstance(raw_items, list) or not raw_items:
        raise WorkspaceCampaignError(
            "sanitized feedback items must be a non-empty list"
        )
    try:
    ...
    if len(ids) != len(set(ids)):
        raise WorkspaceCampaignError(
            "sanitized feedback batch contains duplicate candidates"
        )
    if any(item.campaign_round != source_round for item in items):
        raise WorkspaceCampaignError(
            "feedback item campaign round differs from batch source round"
        )
...
def validate_sanitized_feedback_bindings(
    ...
    for item in items:
        if item.campaign_round >= campaign_round:
            raise WorkspaceCampaignError(
                "feedback must come from a prior campaign round"
            )
        try:
...
def _latest_checkpoint(
    repository: SQLiteRepository, run_id: str
) -> tuple[EventRecord, CampaignCheckpoint] | None:
...
def _resolved_usage(
    ...
    if previous.candidates > inferred.candidates:
        raise WorkspaceCampaignError(
            "checkpoint candidate usage exceeds repository ledger"
        )
    return BudgetUsage(
        candidates=inferred.candidates,
        generations_started=max(
            previous.generations_started, inferred.generations_started
        ),
...
def _record_checkpoint(
    ...
    *,
    ...
    event = repository.record_event(
        run_id, CAMPAIGN_CHECKPOINT_EVENT, checkpoint.to_dict()
    )
    return event, checkpoint
...
def load_campaign_checkpoint(workspace: str | Path) -> CampaignCheckpoint | None:
    ...
    with SQLiteRepository(root / descriptor["database"]) as repository:
        ...
        if checkpoint.next_campaign_round != registry.next_campaign_round:
            raise WorkspaceCampaignError(
                "checkpoint next campaign round is inconsistent"
            )
...
def register_replay_generation(
    ...
    *,
    ...
    if any(not candidate.has_explicit_lineage for candidate in proposed):
        raise WorkspaceCampaignError(
            "every replay proposal must declare explicit lineage"
        )
    ...
    with SQLiteRepository(root / descriptor["database"]) as repository:
        ...
        prior = [
            record for record in records if record.spec.campaign_round < campaign_round
        ]
        current = [
            record for record in records if record.spec.campaign_round == campaign_round
        ]
        ...
        if future:
            raise WorkspaceCampaignError(
                "repository already contains a later campaign round"
            )
        ...
        if current:
            ...
            expected = {
                candidate.candidate_id: candidate.content_hash for candidate in proposed
            }
            ...
            full_registry = _registry_from_records(
                descriptor["protocol_hash"], all_records
            )
            ...
            if integrity:
                raise WorkspaceCampaignError(
                    f"repository integrity failed: {integrity}"
                )
