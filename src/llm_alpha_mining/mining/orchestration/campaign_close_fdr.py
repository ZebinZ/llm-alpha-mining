...
class FrozenHypothesis:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "spec_hash", _require_digest(self.spec_hash, "spec_hash")
        )
        object.__setattr__(
            self,
            "semantic_hash",
...
class PrimaryTestAttempt:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, "attempt_id", _require_positive_int(self.attempt_id, "attempt_id")
        )
        object.__setattr__(
            self,
            "attempt_number",
...
class CampaignCloseFDRRequest:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "campaign_id", _require_id(self.campaign_id, "campaign_id")
        )
        for name in (
            "protocol_hash",
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "CampaignCloseFDRRequest":
        ...
        return cls(
            ...
            hypotheses=tuple(
                FrozenHypothesis.from_dict(item) for item in value["hypotheses"]
            ),
            attempts=tuple(
                PrimaryTestAttempt.from_dict(item) for item in value["attempts"]
            ),
        )
...
def benjamini_hochberg_complete_family(p_values: Sequence[float]) -> tuple[float, ...]:
    ...
    if not normalized:
        raise CampaignCloseFDRError(
            "complete-family BH requires at least one hypothesis"
        )
...
class CampaignCloseFDRReport:
    ...
    def __post_init__(self) -> None:
        ...
        if self.family_size <= 0 or self.family_size != len(self.results):
            raise CampaignCloseFDRError(
                "family_size must equal the non-empty result count"
            )
        if any(
            not isinstance(item, CampaignCloseHypothesisResult) for item in self.results
        ):
            raise CampaignCloseFDRError(
                "results must contain only CampaignCloseHypothesisResult values"
            )
        if (
            tuple(sorted(self.results, key=lambda item: item.candidate_id))
            != self.results
        ):
            raise CampaignCloseFDRError(
                "FDR report results must be sorted by candidate_id"
            )
        _require_unique(
            [item.candidate_id for item in self.results], "result candidate_id"
        )
        ...
        if expected_raw_q != tuple(item.raw_bh_q for item in self.results):
            raise CampaignCloseFDRError(
                "raw BH q-values do not match the complete family"
            )
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "CampaignCloseFDRReport":
        ...
        for key, expected_value in fixed.items():
            if value[key] != expected_value:
                raise CampaignCloseFDRError(
                    f"invalid fixed report governance field: {key}"
                )
        ...
        report = cls(
            ...
            invalid_neutral_primary_test_count=value[
                "invalid_neutral_primary_test_count"
            ],
...
def build_campaign_close_fdr_report(
    ...
    raw_q = benjamini_hochberg_complete_family([item["raw_p"] for item in intermediate])
...
def verify_campaign_close_fdr_report(
    ...
    if report.to_dict() != expected.to_dict():
        raise CampaignCloseFDRError(
            "report differs from deterministic FDR recomputation"
        )
