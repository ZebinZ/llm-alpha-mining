...
class EvaluationAttemptStore:
    ...
    def _initialize_schema(self) -> None:
        ...
        existing = dict(
            self.connection.execute(
                "SELECT key, value FROM attempt_metadata"
            ).fetchall()
        )
        if not existing:
    ...
    def reserve(
        ...
        *,
        ...
        if (
            ...
            or float(estimated_wall_seconds) <= 0
        ):
    ...
    def heartbeat(
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            if cursor.rowcount != 1:
                raise AttemptLeaseConflict(
                    "running attempt is not owned by this worker"
                )
        return self.get_attempt(attempt_id)
        ...
        *,
        ...
        with self._transaction() as connection:
            ...
            if (
                ...
                raise AttemptLeaseConflict(
                    "checkpoint writer does not own a running attempt"
                )
            existing = {
                row["stage"]
    ...
    def verify_integrity(self) -> tuple[str, ...]:
        ...
        for row in self.connection.execute("SELECT * FROM evaluation_attempts"):
            ...
            if (
                row["result_hash"] is not None
                and _SHA256_RE.fullmatch(row["result_hash"]) is None
            ):
