...
class StopController:
    ...
    def observe(
        ...
        if observation.generation <= self._last_generation:
            raise ValueError(
                "objective observations must have strictly increasing generations"
            )
        ...
        if budget_reason is not StopReason.CONTINUE:
            return StoppingDecision(
                True, budget_reason, "a pre-registered hard budget was exhausted"
            )

        if (
            self._best is None
            or observation.value > self._best + self.config.min_improvement
        ):
            self._best = observation.value
            self._stalled = 0
            return StoppingDecision(
                False, StopReason.CONTINUE, "local objective improved"
            )
        self._stalled += 1
        ...
        return StoppingDecision(
            False, StopReason.CONTINUE, "within pre-registered patience"
        )
