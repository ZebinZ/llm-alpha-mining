...
class SanitizedFeedback:
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ValueError(
                f"unsupported feedback disposition: {self.disposition!r}"
            ) from exc
...
class ProposalContext:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            not isinstance(item, SanitizedFeedback) for item in self.sanitized_feedback
        ):
            raise ValueError(
                "sanitized_feedback accepts SanitizedFeedback objects only"
            )
        feedback_ids = [item.candidate_id for item in self.sanitized_feedback]
        if len(feedback_ids) != len(set(feedback_ids)):
            raise ValueError(
                "sanitized_feedback must contain at most one item per candidate"
            )
...
class ProposalBatch:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            raise ValueError("proposal batch accepts CandidateSpec objects only")
