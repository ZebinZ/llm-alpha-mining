...
from llm_alpha_mining.mining.artifacts import (
    ArtifactManifest,
    ArtifactStore,
    canonical_json_bytes,
    hash_json,
)
from llm_alpha_mining.mining.artifacts.hashing import hash_bytes, hash_file
from llm_alpha_mining.mining.candidate_protocol_authority import (
    CandidateProtocolAuthority,
)
from llm_alpha_mining.mining.domain.enums import RunState
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="alpha-workspace",
        description="Initialize and inspect reproducible factor research workspaces",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    init = subparsers.add_parser(
        "init", help="initialize and freeze a research run workspace"
    )
    init.add_argument("--workspace", required=True)
    init.add_argument("--protocol")
    init.add_argument("--run-id")
    for name in ("status", "verify"):
        command = subparsers.add_parser(name, help=f"{name} a research run workspace")
