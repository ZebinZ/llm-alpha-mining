...
from llm_alpha_mining.mining.domain.models import CANDIDATE_SPEC_SCHEMA
from llm_alpha_mining.mining.orchestration.budget import BudgetLimits
from llm_alpha_mining.mining.orchestration.stopping import StoppingConfig
...
class ExternalEvaluationPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.feedback_policy != "holdout_only":
            raise ProtocolError(
                "external evaluation feedback_policy must be 'holdout_only'"
            )
...
class MiningProtocol:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.max_expression_depth, int)
            or self.max_expression_depth <= 0
        ):
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, Any]) -> "MiningProtocol":
        expected = {
            "schema_version",
        ...
        if unknown or missing:
            raise ProtocolError(
                f"protocol fields invalid; unknown={unknown}, missing={missing}"
            )
        stopping = value["stopping"]
        if set(stopping) != {"patience_generations", "min_improvement"}:
            raise ProtocolError(
                "stopping fields must be patience_generations and min_improvement"
            )
        return cls(
            ...
            external_evaluation=ExternalEvaluationPolicy.from_dict(
...
def load_protocol(
    path: str | Path, *, expected_file_hash: str | None = None
) -> MiningProtocol:
    source = Path(path)
    if expected_file_hash is not None:
        ...
        if actual != expected_file_hash:
            raise ProtocolError(
                f"protocol file hash mismatch: expected {expected_file_hash}, got {actual}"
            )
    try:
    ...
    return MiningProtocol.from_dict(payload)


def write_protocol(protocol: MiningProtocol, path: str | Path) -> Path:
...
def default_protocol(protocol_id: str) -> MiningProtocol:
    return MiningProtocol(
