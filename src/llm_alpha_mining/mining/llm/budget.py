...
from llm_alpha_mining.mining.llm.domain import Usage
...
class LLMBudget:
    ...
    def from_ledger(cls, limits: LLMBudgetLimits, ledger: "CallLedger") -> "LLMBudget":
        ...
        if exceeded:
            raise LLMBudgetExceeded(
                "recovered LLM budget already exceeds limits: " + ",".join(exceeded)
            )
        return budget
        ...
        *,
        ...
        with self._lock:
            if self._ledger is not None:
                raise RuntimeError(
                    "bound budgets reserve atomically through CallLedger"
                )
    ...
    def settle(self, reservation_id: str, usage: Usage) -> None:
        with self._lock:
            ...
            if usage.output_tokens > reserved["output_tokens"]:
                raise LLMBudgetExceeded(
                    "provider output usage exceeded its reservation"
                )
    ...
    def release_known_failure(self, reservation_id: str) -> None:
        with self._lock:
            if self._ledger is not None:
                raise RuntimeError(
                    "bound budgets finalize atomically through CallLedger"
                )
            self._reservations.pop(reservation_id)
