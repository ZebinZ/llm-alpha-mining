...
def _collapsed(value: str) -> str:
    ...
    return "".join(
        character for character in security_normalize(value) if character.isalnum()
    )
...
def assert_safe_context(value: Any, *, path: str = "$") -> None:
        ...
        for key, item in value.items():
            ...
            if any(_collapsed(token) in compact for token in _TEACHER_TOKENS):
                raise UnsafeLLMContext(
                    f"teacher/official field forbidden at {path}.<field>"
                )
            if any(
                ...
                raise UnsafeLLMContext(
                    f"raw identifier or exact metric forbidden at {path}.<field>"
                )
            if any(token in compact for token in _SECRET_COMPACT_TOKENS):
                raise UnsafeLLMContext(
                    f"credential metadata forbidden at {path}.<field>"
                )
    ...
    if isinstance(value, str):
        ...
        if any(
            token in compact for token in _SECRET_COMPACT_TOKENS
        ) or _CREDENTIAL_PREFIX.search(value):
        ...
        if _COT_OR_INJECTION.search(normalized) or any(
            ...
            raise UnsafeLLMContext(
                f"reasoning/prompt-injection text forbidden at {path}"
            )
...
def assert_safe_response_metadata(*, model_id: str, provider_request_id: str) -> None:
    ...
    assert_safe_context(
        {"model_identifier": model_id, "provider_identifier": provider_request_id}
    )
...
class ParentCatalogEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not isinstance(self.lineage_depth, int)
            or isinstance(self.lineage_depth, bool)
            or self.lineage_depth < 0
        ):
            raise UnsafeLLMContext("invalid parent lineage_depth")
        if (
            not isinstance(self.expression, str)
            or not self.expression
            or len(self.expression) > 1024
        ):
        ...
        if self.canonical_expression_ast != expected_ast:
            raise UnsafeLLMContext(
                "parent canonical_expression_ast does not bind expression"
            )
        ...
        if (
            len(self.canonical_expression_ast) > 4096
        ...
        if (
            ...
            or any(
                re.fullmatch(r"[A-Za-z][A-Za-z0-9_]*", item) is None for item in fields
            )
        ):
            raise UnsafeLLMContext(
                "parent required_fields must be non-empty and unique"
            )
        ...
        if self.semantic_hash != expected_semantic:
            raise UnsafeLLMContext(
                "parent semantic_hash does not bind safe executable fields"
            )
    ...
    def from_draft(
        ...
        *,
        ...
        if spec_hash is not None and spec_hash != expected:
            raise UnsafeLLMContext(
                "supplied parent spec_hash does not bind draft fields"
            )
...
class CandidateDescriptor:
    ...
    def __post_init__(self) -> None:
        ...
        if self.canonical_expression_ast != expected_ast:
            raise UnsafeLLMContext(
                "descriptor AST does not bind authoritative expression"
            )
        if self.mechanism_code not in set(MECHANISM_CODES):
        ...
        if len(parents) != len(set(parents)) or any(
            _CANDIDATE_ID.fullmatch(item) is None for item in parents
        ):
        ...
        if self.semantic_hash != expected_semantic:
            raise UnsafeLLMContext(
                "descriptor semantic_hash does not bind authoritative fields"
            )
...
def _validate_review_items(
    ...
    for item in values:
        ...
        if (
            _CANDIDATE_ID.fullmatch(item["candidate_id"]) is None
...
def validate_safe_context_payload(value: Mapping[str, Any]) -> None:
    ...
    descriptor_entries = tuple(
        CandidateDescriptor.from_dict(item) for item in descriptors
    )
    ...
    if len({item.semantic_hash for item in descriptor_entries}) != len(
        descriptor_entries
    ):
...
class SafeResearchContext:
    ...
    def __post_init__(self) -> None:
        ...
        if self.generation > 0 and not catalog:
            raise UnsafeLLMContext(
                "derived generations require a closed parent catalog"
            )
    ...
    def to_dict(
        ...
        *,
        ...
        descriptors = [
            CandidateDescriptor.from_draft(item).to_dict() for item in candidate_drafts
        ]
        payload = {
