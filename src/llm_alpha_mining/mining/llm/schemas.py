...
from llm_alpha_mining.mining.llm.domain import LLMRole, thaw_json
...
def reject_chain_of_thought(value: Any, *, path: str = "$") -> None:
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            normalized = (
                _security_normalize(str(key)).strip().casefold().replace(" ", "_")
            )
            if normalized in _REASONING_KEYS:
                raise StructuredOutputError(
                    f"chain-of-thought field forbidden at {path}.{key}"
                )
...
def validate_strict_json(
    ...
    if expected_type == "object":
        ...
        if not isinstance(properties, Mapping):
            raise StructuredOutputError(
                f"invalid schema: {path} object has no properties"
            )
        ...
        if unknown or missing:
            ...
            raise StructuredOutputError(
                f"{path} object fields are outside the closed schema"
            )
        for key, item in value.items():
    ...
    if expected_type == "array":
        ...
        if not isinstance(item_schema, Mapping):
            raise StructuredOutputError(
                f"invalid schema: {path} array has no item schema"
            )
        for index, item in enumerate(value):
...
def _decision_item(
    *, decision_name: str, decisions: list[str], codes: list[str]
) -> dict[str, Any]:
    return {
        "type": "object",
        "additionalProperties": False,
...
class CandidateDraft:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "CandidateDraft":
        ...
        if "\n" in hypothesis or "\r" in hypothesis:
            raise StructuredOutputError(
                "hypothesis must be a compact deliverable, not process notes"
            )
        if re.search(
            ...
            raise StructuredOutputError(
                "hypothesis contains reasoning-process narration"
            )
        return cls(
...
def canonical_aggregation(
    frequency: str, aggregation: Mapping[str, Any]
) -> dict[str, Any]:
...
def _canonicalize_expression_node(node: ast.AST) -> ast.AST:
    ...
    if isinstance(node, ast.Call):
        ...
        if name in _COMMUTATIVE_OPERATORS:
            ...
            for item in args:
                collect(item)
            flattened.sort(
                key=lambda item: ast.dump(
                    item, annotate_fields=True, include_attributes=False
                )
            )
            if len(flattened) >= 2:
                rebuilt = ast.Call(
    ...
    if isinstance(node, ast.UnaryOp):
        return ast.UnaryOp(
            op=node.op, operand=_canonicalize_expression_node(node.operand)
        )
...
def parse_role_output(role: LLMRole | str, payload: Mapping[str, Any]) -> Any:
    ...
    if normalized is LLMRole.CRITIC:
        items = tuple(
            Review(
                item["candidate_id"],
                CriticVerdict(item["verdict"]).value,
        ...
        items = tuple(
            Review(
                item["candidate_id"],
                RiskDecision(item["decision"]).value,
                tuple(item["reason_codes"]),
            )
    ...
    if len(ids) != len(set(ids)):
        raise StructuredOutputError(
            f"{normalized.value} emitted duplicate candidate IDs"
        )
    if normalized is LLMRole.RISK:
        for item in items:
            ...
            if item.decision == RiskDecision.ALLOW.value and codes != {"no_blocker"}:
                raise StructuredOutputError(
                    "risk allow requires exactly the no_blocker code"
                )
            if item.decision == RiskDecision.BLOCK.value and "no_blocker" in codes:
                raise StructuredOutputError(
                    "risk block cannot carry the no_blocker code"
                )
    return items
