...
from llm_alpha_mining.mining.llm.budget import LLMBudget
...
from llm_alpha_mining.mining.llm.ledger import CallLedger, IndeterminateCallError
...
class StructuredCallExecutor:
    ...
    def _validate_response(
        self, request: StructuredCallRequest, response: StructuredCallResponse
    ) -> None:
        try:
            validate_response_against_request(
                request,
                ...
                try:
                    ...
                    if exc.dispatch_unknown:
                        self.ledger.indeterminate(
                            request,
                            attempt=attempt,
                            error_code="transport_dispatch_indeterminate",
                        )
                        ...
                        self.ledger.retryable_failure(
                            request,
                            attempt=attempt,
                            error_code="transport_retryable_failure",
                        )
                        attempt += 1
                        retry_dispatch = True
                    else:
                    ...
                    outward_failure = StructuredCallFailed("response_dlp_rejected")
                except Exception:
                    self.circuit.failure()
