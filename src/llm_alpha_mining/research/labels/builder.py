...
from llm_alpha_mining.research.labels.spec import LabelSpec, LabelTask
from llm_alpha_mining.research.labels.view import LabelDataView
...
def _daily_index(view: LabelDataView) -> pd.DatetimeIndex:
    ...
    if not local_dates.is_unique:
        raise ValueError("LabelBuilder currently requires one bar per session")
