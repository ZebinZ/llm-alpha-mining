...
from llm_alpha_mining.research.core.hashing import canonical_json_bytes, require_sha256
from llm_alpha_mining.research.factors.spec import FactorSpec
    ...
    def register_many(
        ...
        *,
        ...
        try:
            with self._transaction() as connection:
                return tuple(
                    self._register_locked(connection, spec, timestamp) for spec in specs
                )
        except sqlite3.IntegrityError as exc:
            raise FactorRegistryConflict(
                "factor id/version or executable definition already exists"
