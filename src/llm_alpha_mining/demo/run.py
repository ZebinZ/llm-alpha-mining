...
from llm_alpha_mining.research.evaluation.diagnostics import (
...
from llm_alpha_mining.mining.llm.agents import StructuredCallExecutor
...
from llm_alpha_mining.mining.providers.base import ProposalContext
...
def scripted_roles() -> dict:
    ...
    return {
        LLMRole.PROPOSER: [
            {"schema_version": "llm-proposer-output/v1", "proposals": proposals}
        ],
        LLMRole.CRITIC: [
            {
                ...
                "assessments": [
                    {
                        "candidate_id": p["candidate_id"],
                        "decision": "block"
                        if p["candidate_id"] == "risk_blocked_example"
                        else "allow",
                        "reason_codes": [
                            "unsafe_semantics"
                            if p["candidate_id"] == "risk_blocked_example"
...
def synthetic_view(out: Path, seed: int):
    ...
    timestamps = pd.bdate_range(
        "2020-01-02", periods=100, tz="Asia/Shanghai"
    ) + pd.Timedelta(hours=15)
    dates = tuple(timestamps.strftime("%Y%m%d"))
    securities = tuple(f"{i:06d}" for i in range(1, 41)) + ("399001",)
    returns = pd.DataFrame(
        rng.normal(0, 0.012, (100, 41)), index=dates, columns=securities
    )
    returns.iloc[30:33, 3] = np.nan
    amount = pd.DataFrame(
        rng.lognormal(14, 0.4, (100, 41)), index=dates, columns=securities
    )
    ...
    dump(
        source,
        {
            ...
            "returns_hash": hash_frame(returns),
            "amount_hash": hash_frame(amount),
        },
    )
    schema = standard_market_schema(
        schema_id="synthetic-daily",
        version="1",
    ...
    dataset = DatasetSpec(
        ...
        version="1",
        owner="demo",
        description="Generated demonstration; no real market observations",
    ...
    snapshot = DatasetSnapshot.create(
        ...
        source_assets=(
            SourceAsset.from_path(
                source,
                logical_name="synthetic-source",
                expected_sha256=hash_file(source),
            ),
        ),
        source_vintage_at="2020-12-30T00:00:00+08:00",
        created_at="2020-12-31T00:00:00+08:00",
    )
    request = DataRequest(
        ...
        as_of="2020-12-31T00:00:00+08:00",
        role=DataRole.RESEARCH,
        frequency=frequency,
    )
    frame = pd.DataFrame(
        {
            "timestamp": timestamps.repeat(len(securities)),
            "security": list(securities) * len(dates),
            "Returns": returns.to_numpy().ravel(),
            "Amount": amount.to_numpy().ravel(),
        }
    )
    ...
    events = pd.DataFrame(
        [
            {
                "instrument_id": s,
                "instrument_type": "index" if s == "399001" else "stock",
                "exchange": "XSHE",
                "status": "ACTIVE",
    ...
    contract = build_security_data_contract(
        ...
        signal_universe_mask=universe,
        stock_returns=returns,
        stock_close=pd.DataFrame(10.0, index=dates, columns=securities),
        stock_amount=amount,
    ...
    view = FactorDataView.from_market_view(
        market, contract, required_fields=("Returns", "Amount")
    )
...
def run_demo(output: Path, *, seed: int = 42) -> dict:
    ...
    executor = StructuredCallExecutor(
        transport=fake,
        ledger=CallLedger(output / "llm_calls.jsonl"),
        budget=LLMBudget(
            LLMBudgetLimits(
                max_calls=4,
    ...
    for candidate in batch.candidates:
        spec = factor_spec_from_candidate(
            candidate,
            economic_rationale="Synthetic illustration of short-lived price pressure.",
    ...
    labels = labels.reindex(
        index=next(iter(factors.values())).index,
        columns=next(iter(factors.values())).columns,
    )
    domain = view.cross_section_mask.reindex(index=labels.index, columns=labels.columns)
    diagnostics = build_descriptive_smoke_diagnostics(
        factors=factors,
        labels=labels,
        factor_domain_masks={name: domain for name in factors},
        full_universe_masks=domain,
        label_validity=np.isfinite(labels),
        minimum_observations=10,
    )
    diagnostics.verify_content()
    diagnostics.per_date.to_parquet(output / "descriptive_metrics.parquet")
    diagnostics.pairwise_factor_correlation.to_parquet(
        output / "factor_correlations.parquet"
    )
    dump(output / "candidate_specs.json", specs)
    scientific = {
        "seed": seed,
        "factor_hashes": {name: hash_frame(f) for name, f in sorted(factors.items())},
        "diagnostics_hash": diagnostics.content_hash,
    }
    report = {
        "status": "PASS",
        "synthetic_data": True,
        "transport": "fake-offline",
        "network_calls": 0,
        "scripted_role_calls": fake.call_count,
        "proposed": 3,
        "accepted": sorted(factors),
        "risk_veto_demonstrated": "risk_blocked_example" not in factors,
        "dates": len(labels),
        "synthetic_stocks": 40,
        "excluded_index": "399001",
        "formal_inference_permitted": False,
        "admission_decision_permitted": False,
        "scientific_signature": hash_json(scientific),
        **scientific,
    }
    ...
    dump(
        ...
        {
            "schema": "offline-demo-artifacts/v1",
            "files": [
                {
...
def main() -> None:
    ...
    print(
        json.dumps(
            verify(args.output)
            if args.verify
            else run_demo(args.output, seed=args.seed),
            indent=2,
        )
    )
